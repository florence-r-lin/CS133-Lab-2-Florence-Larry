...
public class ScanTest extends SimpleDbTestBase {
    ...
    @Test public void testTupleDesc() throws IOException, DbException, TransactionAbortedException {
        ArrayList<ArrayList<Integer>> tuples = new ArrayList<ArrayList<Integer>>();
        int numcols = 3;
        HeapFile f = SystemTestUtil.createRandomHeapFile(numcols, 1000, null, tuples);
        ...
        assertEquals("TupleDesc should have "+numcols+" fields", td.numFields(),numcols);
        for (int i = 0; i < numcols; i++) {
        assertTrue("Each field name in tuple desc should be prefixed with alias",
          td.getFieldName(i).startsWith("TheAlias"));
        }
