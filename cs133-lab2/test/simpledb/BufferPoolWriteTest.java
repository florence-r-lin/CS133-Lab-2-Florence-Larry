...
public class BufferPoolWriteTest extends TestUtil.CreateHeapFile {
    ...
    @Test public void insertTuple() throws Exception {
        ...
        for (int i = 0; i < 504; ++i) {
          ...
          assertNotNull("BufferPool insert should mark pages as dirty", p.isDirty());
        }
    ...
    @Test public void deleteTuple() throws Exception {
    	...
    	for (int i = 0; i < 504; ++i) {
			...
			PageId pageDeletedFrom = t.getRecordId().getPageId();
        	Database.getBufferPool().deleteTuple(tid, t);
        	HeapPage p = (HeapPage) Database.getBufferPool().getPage(tid, pageDeletedFrom, Permissions.READ_ONLY);
        	assertEquals(i+1, p.getNumEmptySlots());
          assertNotNull("BufferPool delete should mark pages as dirty", p.isDirty());
        }
