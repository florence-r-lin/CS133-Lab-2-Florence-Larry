...
public class HeapFileWriteTest extends TestUtil.CreateHeapFile {
    ...
    @Test public void addTuple() throws Exception {
      ...
      for (int i = 0; i < 504; ++i) {
          ArrayList<Page> pages = empty.insertTuple(tid, Utility.getHeapTuple(i, 2));
          assertEquals(1, empty.numPages());
          
         // the HeapFile's insertTuple shouldn't mark pages as dirty                
         //(leave that to the buffer pool's insertTuple)      
        assertNull("Call markDirty in BufferPool's insertTuple(), not HeapFile",
                     pages.get(0).isDirty());
        }
